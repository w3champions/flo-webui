import { Button, Callout, Classes, Divider, H1, H2, Icon, Intent, NonIdealState, Tab, Tabs, Tag } from "@blueprintjs/core";

import { SubscriptionHandler, useClient, useSubscription } from 'urql'
import { GameUpdateSubSubscription, GameUpdateSubDocument, GameSnapshotFieldsFragment, GameStatsSnapshotFieldsFragment, PingFieldsFragment, ActionFieldsFragment, PlayerLeaveReason } from "../../generated/graphql";
...
const handleSubscription: SubscriptionHandler<GameUpdateSubSubscription, State> = (state = initialState, message) => {
  if (!message) {
    return initialState
  }
...
export default function LiveGame() {
  ...
  const [res] = useSubscription({ query: GameUpdateSubDocument, variables: { id: Number(router.query.id) } }, handleSubscription)
  ...
  return (
    <>
      <main className={`${Classes.DARK}`}>
        ...
        ></NonIdealState>}
        {res.data?.game && res.data?.stats && <Game game={res.data.game} stats={res.data.stats} />}
      </main>
      <style jsx global>{`
        body { padding: 16px; }
      `}</style>
...
function Game({ game, stats }: { game: Game, stats: Stats }) {
  const playerStats = useMemo(() => {
    const ping = stats.ping.length > 0 ? stats.ping[stats.ping.length - 1] : null
    const action = stats.action.length > 0 ? stats.action[stats.action.length - 1] : null
  ...
  return <div className="flex flex-col w-full space-y-4">
      ...
      <div className="flex-auto flex space-x-16">
        <div className="flex-auto space-y-4">
          <h3 className={`${Classes.HEADING} flex flex-row space-x-2`}>
            ...
            <Tag className={game.endedAt ? '' : 'animate-pulse'}>{game.endedAt ? `Ended at ${format(new Date(game.endedAt), 'yyyy-MM-dd HH:mm')}` : running || 'Running'}</Tag>
          </h3>

          <div className="space-y-2">
            <div className="flex">
              <span className="flex-initial">
                Started at {format(new Date(game.startedAt), 'yyyy-MM-dd HH:mm')}
              </span>
            </div>
            <div className="flex">
              <span className="flex-initial">
    ...
    <div className="flex-auto space-y-1">
      {game.players.map(({ id, name, race, team, leftAt, leaveReason }, idx) => {
        ...
        return (
          ...
          >
            ...
            {leaveReason && <div className="w-36">
              <Tag intent={leaveReason == PlayerLeaveReason.LeaveWon ? Intent.SUCCESS : Intent.WARNING} title={`${(leftAt / 1000).toFixed(2)}s`}>{leaveReason}</Tag>
            </div>}
            <div className="w-36">
    ...
    <Divider className="flex-initial" />
    <Tabs selectedTabId={activaTab} onChange={(e) => { setActiveTab(e as ChartTabs) }}>
      <Tab id={ChartTabs.Ping} title="Ping" panel={<div className="chart"><Line
        height={111}
        datasetIdKey='id'
        data={{
      ...
      /></div>} />
      <Tab id={ChartTabs.APM} title="APM" panel={
        <div className="chart">
          <Line
            datasetIdKey='id'
            data={{
